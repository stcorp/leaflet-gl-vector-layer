...
export class ColorBar {
  ...
  private initialize() {
    ...
    this.controlsService.layerSelected$.pipe(takeUntil(this.destroyed$)).subscribe(layer => {
    ...
    this.controlsService.limits$.pipe(takeUntil(this.destroyed$)).subscribe(limits => {
      ...
      const range = max - min;
      const tickCount = 4;
      const tickWidth = this.bar.width / tickCount;
      for (let i = 0; i < tickCount + 1; i++) {
        const tickWrapper = L.DomUtil.create('div', 'tick-wrapper', this.tickContainer);
    ...
    this.colorService.gradient$.pipe(takeUntil(this.destroyed$)).subscribe(gradientData => {
      ...
      if (context) {
        ...
        const gradient = context.createLinearGradient(0, 0, this.bar.width, 0);
        for (let i = 0; i <= 255; i++) {
          const color = gradientData.gradient(i / 255).rgba();
  ...
  private resizeBar() {
    ...
    this.wrapper.style.transform = `scale(${Math.min(newScale, 1)}) translateX(${
      -100 * (1 - Math.min(newScale, 1))
    }px)`;
  }

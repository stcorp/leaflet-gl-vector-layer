import { Browser, Bounds, DomUtil, LatLng, Map, setOptions, Layer, Point, LatLngBounds } from 'leaflet';
...
export class LeafletGlVectorLayer extends L.GridLayer {
  ...
  public onAdd(map: Map) {
    ...
    this.renderer = new RendererMap[this.options.leafletGlVectorLayerOptions.plot_type](
      this.options.leafletGlVectorLayerOptions,
      map,
      this.dataHelper,
      this.canvas
    );
    ...
    if (this.isAnimated()) {
      ...
      map.on(zoomhandler.type, zoomhandler.func, this);
    ...
    this.controlsService.hideLayer$
      .pipe(
    ...
    this.controlsService.showLayer$
      .pipe(
        takeUntil(this.destroyed$),
        filter((layer: LeafletGlVectorLayer) => {
          return layer.id === this.id;
  ...
  private _redraw(): void {
    ...
    if (this.renderer) {
      const adjustedTopLeft = new LatLng(topLeft.lat, topLeft.lng);
  ...
  private _animateZoom(e: any): void {
    ...
    const offset = this._unclampedLatLngBoundsToNewLayerBounds(_map.getBounds(), e.zoom, e.center).min;
  ...
  private _animateZoomNoLayer(e: any): void {
    ...
    if (this.canvas) {
      const scale = _map.getZoomScale(e.zoom, _map.getZoom());
      const offset = _map._getCenterOffset(e.center)._multiplyBy(-scale).subtract(_map._getMapPanePos());
      DomUtil.setTransform(this.canvas, offset, scale);
    }
  }

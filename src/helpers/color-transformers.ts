...
export function colorWrappersToXrgbaColormap(colorWrappers: IColorEdgePoint[]): IXRGBA[] {
  return colorWrappers.map((colorEdgePoint: IColorEdgePoint, index: number) => {
    return [
      colorEdgePoint.value,
      colorEdgePoint.color[0] / 255,
      colorEdgePoint.color[1] / 255,
      colorEdgePoint.color[2] / 255,
      colorEdgePoint.color[3],
    ] as IXRGBA;
  });
}
...
export function getGradientForEdgePoints(colorWrappers: IColorEdgePoint[]): chroma.Scale {
  const colors = getColorScaleString();
  const gradient = chroma.scale(colors).domain([...colorWrappers.map(position => position.value)]);
  return gradient;

  function getColorScaleString() {
    const colors = colorWrappers.map(item => {

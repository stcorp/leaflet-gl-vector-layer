...
export class SwathRenderer extends BaseRenderer {
  ...
  public processData(callback: () => void) {
    ...
    for (let index = 0; index < this.data.latitudes.length; index += 4) {
      if (!values[index / 4] && values[index / 4] !== 0) {
        continue;
      }
      const value = this.normalizeValue(values[index / 4]);
      const longitudesForSquare = [
        this.data.longitudes[index],
        this.data.longitudes[index + 1],
        this.data.longitudes[index + 2],
        this.data.longitudes[index + 3],
      ];

      const isCrossAntiMeridian = longitudesForSquare.some(
        longitude => longitude <= -160 && longitudesForSquare.some(longitude => longitude >= 60)
      );
      ...
      if (isCrossAntiMeridian) {
        const polygons = this.cutPolygon(polygon);
